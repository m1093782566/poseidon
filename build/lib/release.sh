...
function kube::release::package_server_tarballs() {
  ...
  for platform in "${KUBE_SERVER_PLATFORMS[@]}"; do
    ...
    poseidon::release::build_poseidon_image "${arch}" "${docker_registry}" \
    ...
    #copy out poseidon server image
...
function poseidon::release::build_poseidon_image() {
  local -r arch="$1"
  local -r registry="$2"
  local -r version="$3"
  local -r save_dir="${4-}"
  kube::log::status "Building poseidon server image for arch: ${arch}"
  ARCH="${arch}" REGISTRY="${registry}" VERSION="${version}" \
  ...
  if [[ -z "${KUBE_DOCKER_IMAGE_TAG-}" || -z "${KUBE_DOCKER_REGISTRY-}" ]]; then
    # not a release
    kube::log::status "Deleting poseidon server image ${poseidon_tag}"
